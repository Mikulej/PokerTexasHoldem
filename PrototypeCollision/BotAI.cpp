...
Point::Point(const std::vector<Karta>& _hand_cards, const std::vector<Karta>& _desk_cards, bool _isDealer, int _bot_credits,int _bot_gave, int _enemy_credits, int _enemy_gave, bool _enemy_checks,  int _pool,int _checked_cards) {//head constructor
	...
	while (simulations < SIM_NUM) {
		//std::cout << "SIM:"<<simulations << std::endl;
...
bool Point::simulation(void) {
	...
	while (players[0].get_credits()!=0 || players[1].get_credits() !=0) {//bot wykonuje pierwszy ruch
		...
		if (round_start) {		
			...
			if (players[0].get_credits() == 0) { players[0].allin = true; }
			if (players[1].get_credits() == 0) { players[1].allin = true; }
			round_start = false;
		}
		if (missing_blind) { break; }
		//std::cout << "Bot credits=" << players[1].get_credits() << " Player credits=" << players[0].get_credits() << " sim_pool=" << sim_pool << " all=" << players[1].get_credits() + players[0].get_credits() + sim_pool << std::endl;
		if ((*enemy).allin) {
			//if (bot_turn) { std::cout << "Bot answers to all in" << std::endl; }
			//else { std::cout << "Player answers to all in" << std::endl; }
			last_move = true;
		}	
		//perform random action
		int option;
		if (!updated_sim_data) { option = first_move;  } //wykonaj pierwszy ruch zaplanowany w klasie Point
		else//moze ustawic jakies lepsze wagi? ze np fold ma 65% szans	
		{ 
			if ((*enemy).allin) {//bot nie bedzie podbijal stawki gdy gracz poszedl all in - nie ma sensu podbijac stawki
				option = (rand() % 2) + 1;
			}
			else {
				option = rand() % 3;
		...
		case 0://raise - done
			...
			if (raise_amount == (*player).get_credits()) { (*player).allin = true; }
